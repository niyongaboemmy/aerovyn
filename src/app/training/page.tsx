...
import { CheckCircle2, Clock, Award, ChevronDown, Users, Shield, Cpu, ArrowRight } from 'lucide-react'
...
const certifications = [
  { icon: Shield, label: 'RCAA Accredited', desc: 'Rwanda Civil Aviation Authority approved' },
  { icon: Award, label: 'KCAA Recognised', desc: 'Kenya CAA cross-border endorsement' },
  { icon: Cpu, label: 'DJI Authorised', desc: 'Official DJI training partner, East Africa' },
  { icon: Users, label: '500+ Graduates', desc: 'Alumni across 10 African countries' },
]

function CourseRow({ course, expanded, onToggle }: { course: typeof courses[0]; expanded: boolean; onToggle: () => void }) {
  const { level, levelColor, title, duration, image, summary, features, modules, outcomes, prereq } = course
    ...
    >
      ...
      >
        <div className="flex items-stretch gap-0">
          {/* Thumbnail */}
          <div className="relative hidden h-auto w-48 shrink-0 overflow-hidden sm:block" style={{ minHeight: '120px' }}>
            <Image
              src={image}
              alt={title}
              fill
            ...
            <div
              ...
              style={{ background: `linear-gradient(to right, transparent 60%, #111318 100%)` }}
            />
          </div>
            ...
            <div className="flex-grow">
              <div className="mb-2 flex flex-wrap items-center gap-3">
                <span
                  ...
                  style={{ background: `${levelColor}15`, color: levelColor, border: `1px solid ${levelColor}35`, fontFamily: 'var(--font-orbitron)' }}
                >
                  {level}
                </span>
              ...
              <h3 className="text-base font-bold leading-snug text-white sm:text-lg" style={{ fontFamily: 'var(--font-orbitron)' }}>
                {title}
              </h3>
              <p className="mt-1.5 line-clamp-1 text-sm text-[#6B7A8D]">{summary}</p>
            </div>
            <div
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition-all duration-300"
              style={{
                background: expanded ? `${levelColor}20` : 'rgba(255,255,255,0.04)',
                border: `1px solid ${expanded ? levelColor + '50' : 'rgba(255,255,255,0.08)'}`,
                transform: expanded ? 'rotate(180deg)' : 'rotate(0deg)',
              }}
            >
              <ChevronDown size={16} style={{ color: expanded ? levelColor : '#6B7A8D' }} />
            </div>
          </div>
        </div>
      </button>
      ...
      {expanded && (
        <div className="border-t" style={{ borderColor: `${levelColor}18` }}>
          ...
          <div className="relative h-56 w-full overflow-hidden sm:h-72">
            ...
            <div
              className="absolute inset-0"
              style={{ background: 'linear-gradient(to top, #111318 0%, rgba(17,19,24,0.4) 50%, transparent 100%)' }}
            />
            <div className="absolute bottom-6 left-6">
              <p className="text-sm text-[#C4CDD8] max-w-xl leading-relaxed">{summary}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 px-6 py-8 md:grid-cols-3">
            {/* Features */}
            <div>
              <p className="mb-4 text-[10px] font-bold uppercase tracking-widest" style={{ color: levelColor, fontFamily: 'var(--font-orbitron)' }}>
                What You'll Learn
              </p>
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                {modules.map((m) => (
                  <li key={m} className="flex items-start gap-2.5 text-sm text-[#8A9BAE]">
                    <ArrowRight size={12} className="mt-0.5 shrink-0" style={{ color: levelColor + 'aa' }} />
                    {m}
                  </li>
                ))}
              </ul>
            ...
            <div className="flex flex-col">
              ...
              <Link
                ...
                style={{ background: `${levelColor}12`, border: `1px solid ${levelColor}35`, color: levelColor }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = `${levelColor}22` }}
...
export default function TrainingPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-base)' }}>
      ...
      <div className="relative overflow-hidden pt-20 pb-20 sm:pt-28 sm:pb-24 md:pb-32">
        <Image
          src="/images/training/intermediate.jpg"
          alt="Drone training"
          fill
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <p className="max-w-2xl text-base leading-relaxed text-[#8A9BAE] sm:text-lg">
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            {certifications.map(({ icon: Icon, label }) => (
              <div
                ...
                style={{ background: 'rgba(0,245,196,0.06)', border: '1px solid rgba(0,245,196,0.18)' }}
              >
                <Icon size={13} className="text-[#00F5C4]" />
                <span className="text-xs font-semibold text-white" style={{ fontFamily: 'var(--font-orbitron)' }}>{label}</span>
              </div>
            ))}
          </div>
        </div>
              ...
              >
                ...
                <div className="mb-4 flex items-center gap-4">
                  <div
                    ...
                    style={{ background: `${accent}18`, color: accent, fontFamily: 'var(--font-orbitron)', border: `1px solid ${accent}30` }}
                  >
                    {initials}
                  </div>
                  <div>
      ...
      <section className="relative overflow-hidden px-4 py-14 sm:px-6 sm:py-16">
        <Image
          ...
          sizes="100vw"
        />
        <div className="relative mx-auto max-w-7xl rounded-2xl px-6 py-12 text-center sm:px-12 sm:py-16"
          style={{ background: 'linear-gradient(135deg, rgba(0,245,196,0.08) 0%, rgba(0,245,196,0.03) 100%)', border: '1px solid rgba(0,245,196,0.2)' }}
        >
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.3em] text-[#00F5C4]" style={{ fontFamily: 'var(--font-orbitron)' }}>
            Next Cohort Open
          </p>
          <h2 className="mb-4 text-3xl font-black tracking-wide text-white sm:text-4xl" style={{ fontFamily: 'var(--font-orbitron)' }}>
            START YOUR JOURNEY TODAY
          </h2>
          <p className="mx-auto mb-10 max-w-md text-sm leading-relaxed text-[#6B7A8D] sm:text-base">
            Secure your spot before it fills. Contact us to discuss the right programme for your goals.
          </p>
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              ...
              style={{ background: '#00F5C4', fontFamily: 'var(--font-orbitron)' }}
            >
              Enroll Now →
            </Link>
            <Link
